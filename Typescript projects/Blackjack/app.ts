...
function playerTurn(playerHand: ICard[], deck: Deck): number {
  let handValue = getHandValue(playerHand);       // Get the value of the player's hand
...
let dealerHand: ICard[] = [];   // Initialize the dealer's hand
let playerHand: ICard[] = [];   // Initialize the player's hand
const deck: Deck = new Deck();
let balance = 100;

while (balance > 0) {
  console.log(`\nPlayer funds $${balance}`);    // Display the player's balance
